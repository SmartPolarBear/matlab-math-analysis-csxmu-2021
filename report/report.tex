...
\begin{equation}
	S(\mathbf{x})=4(x_1+x_2+x_3)-720
\end{equation}

The total cost, say $F(\mathbf{x})$ is the sum of $P(\mathbf{x})$ and $S(\mathbf{x})$ 

\begin{equation}
	F(\mathbf{x})=0.2(x_1^2+x_2^2+x_3^2)+54(x_1+x_2+x_3)-720
\end{equation}

Its standard form for MATLAB to solve is

\begin{equation}
\begin{aligned}
	F(\mathbf{x})=G(\mathbf{x})-720  \\
...
\begin{lstlisting}[style=MatlabStyle,caption=SGD Gradient Descent,label=code:p3]
clear;
% define 32 symbols
p=sym('p',[1,32]);
